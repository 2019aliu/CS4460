...
function updateBars(house) {
    ...
    let filtered_house = [];
    pr_filter = (c) => c['power_ranking'] > 0;
    house_filter = (c) => c['house'] === house;
    if (house === "top") {
        filtered_house = characters.filter(pr_filter);
    } else {
        filtered_house = characters.filter(house_filter);
    }
    // // This is more intuitive, but is long
    // filtered_house.sort((a, b) => a['probability_of_survival'] - b['probability_of_survival']).reverse();

    // This one is less intuitive, but is shorter
    filtered_house.sort((a, b) => b['probability_of_survival'] - a['probability_of_survival']);
    renderBars(filtered_house);
}